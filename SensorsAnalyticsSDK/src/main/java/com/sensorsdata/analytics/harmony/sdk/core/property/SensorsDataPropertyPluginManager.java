...
public final class SensorsDataPropertyPluginManager {
     ...
     */
    public final void properties(String eventName, SAEventType eventType, ZSONObject properties) {
        ...
        try {
            zsonObject = properties(filter(eventName, eventType, properties));
        } catch (Exception e) {
        ...
        SAZSONUtils.mergeZSONObject(zsonObject, properties);
     ...
     * @return 符合条件的插件列表
     */
    private List<SAPropertyPlugin> filter(String eventName, SAEventType eventType, ZSONObject properties) {
     ...
     * @return 是否匹配
     */
    private boolean isMatchPropertyKey(Set<String> propertyKeyFilter, ZSONObject properties) {
     ...
     * @return 是否匹配
     */
    private boolean isMatchEventType(Set<SAEventType> eventTypeFilter, SAEventType eventType) {
        // 如果用户没有实现 eventTypeFilter 添加支持事件类型，则默认只支持 track 类型事件
        if (eventTypeFilter == null || eventTypeFilter.isEmpty()) {
            if (eventType == SAEventType.TRACK) {
                return true;
            }
        }
        // 如果匹配到 EventType.ALL，则对所有类型事件都成功匹配
        if (eventTypeFilter.contains(SAEventType.ALL)) {
            return true;
        }
     ...
     * @return 属性对象
     */
    private ZSONObject properties(List<SAPropertyPlugin> plugins) {
        ZSONObject zsonObject = new ZSONObject();
        if (plugins == null) {
            return zsonObject;
        }
        for (SAPropertyPlugin plugin : plugins) {
            ZSONObject properties = plugin.properties();
            if (properties == null || properties.isEmpty()) {
                continue;
            }
            SAZSONUtils.mergeZSONObject(properties, zsonObject);
        }
        return zsonObject;
    }
     ...
     */
    public final ZSONObject getPropertiesByPlugin(Class<?> clazz) {
        ZSONObject properties = new ZSONObject();
