 ...
 * @author : chenru
 */
public class SATimeUtils {
    public static final String YYYY_MM_DD = "yyyy-MM-dd";
    private static final String YYYY_MM_DD_HH_MM_SS_SSS = "yyyy-MM-dd HH:mm:ss.SSS";
    private static final Map<String, ThreadLocal<SimpleDateFormat>> formatMaps = new HashMap<>();
    ...
    /**
     * 将 ZSONObject 中的 Date 类型数据格式化
     *
     * @param ZSONObject ZSONObject
     * @return ZSONObject
     */
    public static ZSONObject formatDate(ZSONObject ZSONObject) {
        if (ZSONObject == null) {
        ...
        try {
            for (String key : ZSONObject.keySet()) {
                ...
                if (value instanceof Date) {
                    ZSONObject.put(key, formatDate((Date) value, Locale.CHINA));
                }
            }
    ...
    public static boolean isSameDayOfMillis(final long ms1, final long ms2) {
        if (ms1 <= 0 || ms2 <= 0) {
            ...
            if (formatTime(ms1, YYYY_MM_DD).equals(formatTime(ms2, YYYY_MM_DD))) {
                return true;
            }
        }
        return false;
